...
public class ParseSchema {
            ...
            if (ct.getModel() instanceof ModelGroup) {
                ...
                for (SchemaComponent sc : ((ModelGroup) ct.getModel()).getParticles()) {
                    ...
                    if (sc.getClass().getSimpleName().equals("Element")) {
                        out("      Particle Type: " + ((Element) sc).getType());
                        out("      Particle minoccurs: " + ((Element) sc).getMinOccurs());
                        out("      Particle maxoccurs: " + ((Element) sc).getMaxOccurs());
                    }
                    out("");
                }
            }
    ...
    private static void annotationOut(SchemaComponent sc) {
        if (sc.getAnnotation().getAppinfos().size() > 0) {
            System.out.print("    Annotation (appinfos) available with the content: ");
            for (Appinfo appinfo : sc.getAnnotation().getAppinfos()) {
                out(appinfo.getContent());
            }
        } else {
            System.out.print("    Annotation (documentation) available with the content: ");
